...
long2wideClass <- if (requireNamespace('jmvcore', quietly = TRUE))
    R6::R6Class(
        "long2wideClass",
        ...
        private = list(
            ...
            .init = function() {
                ...
                nl <- lapply(indexes, function(x)
                    levels(self$data[[x]]))
                wnames <- lapply(self$options$rowstocols, function(x)
                    combine(nl, prefix = x))
                private$.wnames <- unlist(wnames)
                labs <- lapply(indexes, function(x)
                    paste(x, levels(self$data[[x]]), sep = "="))
            ...
            .reshape = function() {
                ...
                labs <- lapply(seq_along(indexes), function(x)
                    paste(indexes[[x]], levels(data[[indexes[[x]]]]), sep = "="))
                ...
                if (length(indexes) > 1)
                    data$int.index. <- apply(data[, indexes], 1, paste0, collapse =
                                                 "_")
                else
                ...
                alllevels <- as.data.frame(do.call("rbind", lapply(unique(data$id.), function(x)
                    cbind(id. = x, int.index. = levs))))
                ...
                private$.rdata <- private$.rdata[rowSums(is.na(private$.rdata)) <
                                                     (ncol(private$.rdata)), ]
