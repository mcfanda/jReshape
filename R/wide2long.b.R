...
wide2longClass <- if (requireNamespace('jmvcore', quietly = TRUE))
    R6::R6Class(
        "wide2longClass",
        ...
        private = list(
            ...
            .init = function() {
                ...
                jinfo("[W2L .init] phase started")

                self$results$desc$setContent(" ")
                private$.createfile <- self$options$reshape
                if (self$options$mode == "complex") {
                    mark("complex mode")
                    private$.deps <- lapply(self$options$comp_colstorows, function(x)
                        x$label)
                    private$.colstorows <- lapply(self$options$comp_colstorows, function(x)
                        x$vars)
                    indexes <- self$options$comp_index
                    indexes <- indexes[unlist(lapply(indexes, function(x)
                        is.something(trimws(x$var))))]
                    indexes <- indexes[unlist(lapply(indexes, function(x)
                        is.something(x$levels)))]
                    private$.indexes <- indexes
                    private$.indexes_name <- unlist(lapply(indexes, function(x)
                        x$var))
                ...
                if (self$options$mode == "simple") {
                    ...
                    private$.indexes <- list(list(var = self$options$sim_index, levels =
                                                      0))
                ...
                if (!is.something(private$.indexes_name))
                    private$.indexes_name <- "index"
                ...
                test <- any(unlist(lapply(private$.colstorows, function(x)
                    ! is.something(x))))
                if (test) {
                ...
                ns <- unlist(lapply(private$.colstorows, function(x)
                    length(x)))
                ...
                tot <- prod(unlist(lapply(private$.indexes, function(x)
                    as.numeric(x$levels))))
                ref <- ns[1]
                if (length(private$.indexes) > 1) {
                    if (tot != ref) {
                        msg <- paste(
                            "<h2>Help</h2><div>The combination (product) of the index variables levels should be equal
                ...
                if (length(private$.indexes) == 1 &
                    ...
                    self$results$help$setVisible(TRUE)
                    msg <- paste(
                        "<h2>Help</h2><div>Index variable",
                        private$.indexes[[1]]$var,
                        "defined levels are ignored. The number of
                ...
                lapply(private$.tables, function(x)
                    x$initTable())
            ...
            .reshape = function() {
                ...
                if (self$options$mode == "simple") {
                    ...
                    if (length(indexes) > 1 &&
                        self$options$comp_index_values == "name") {
                    ...
                    .levels <- apply(do.call(rbind, colstorows), 2, function(x)
                        paste0(x, collapse = "_"))

                    levels <- switch(
                        self$options$comp_index_values,
                        name  = .levels,
                ...
                if (length(indexes) > 1) {
                    grid <- expand.grid(lapply(indexes, function(x)
                        1:as.numeric(x$levels)))
                    if (length(grep("int.index.", private$.indexes_name, fixed = T)) >
                        0)
                        stop(
                            "'int.index.' is a reserved word, please choose another name for your index variables"
                        )
                    tdata <- as.data.frame(do.call(rbind, lapply(1:private$.on, function(i)
                        grid)))
                    ...
                    names(private$.rdata)[names(private$.rdata) == "int.index."] <- private$.indexes_name
            ...
            .features = function() {
                tab <- aggregate(
                    private$.rdata[[1]],
                    lapply(private$.indexes_name, function(i)
                        private$.rdata[, i]),
                    length
                )
                names(tab)[1:length(private$.indexes)] <- private$.indexes_name
                names(tab)[ncol(tab)] <- "Freq"
                for (i in seq_along(private$.colstorows))
                    tab[[private$.deps[[i]]]] <- private$.colstorows[[i]]
                tab
            },
